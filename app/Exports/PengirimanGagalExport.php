...
class PengirimanGagalExport implements
    FromArray,
    WithColumnWidths,
    WithTitle,
    WithStyles
{
    ...
    public function array(): array
    {
        ...
        $data[] = [!empty($filterInfo) ? 'Filter: ' . implode(' | ', $filterInfo) : 'Filter: Semua Data Pengiriman Gagal', '', '', '', '', '', '', '', '', ''];

        // Baris 4: Kosong
        $data[] = ['', '', '', '', '', '', '', '', '', ''];

        // Header tabel (disamakan dengan export Forecast Gagal)
        $data[] = [
            'Tanggal',
            'Hari',
            'PIC',
        ...
        foreach ($pengirimanList as $pengiriman) {
            ...
            $forecastDetails = collect($pengiriman->forecast->forecastDetails ?? []);
            $keterangan = $pengiriman->catatan ?? 'N/A';
            ...
            if ($pengirimanDetails->isEmpty()) {
                ...
                foreach ($pengirimanDetails as $detail) {
                    ...
                    // Cari forecast detail yang sepadan untuk ambil Qty (qty_kirim di pengiriman gagal selalu null)
                    $matchedForecastDetail = $forecastDetails->first(function ($fd) use ($detail) {
                        return $fd->purchase_order_bahan_baku_id == $detail->purchase_order_bahan_baku_id
                            && $fd->bahan_baku_supplier_id == $detail->bahan_baku_supplier_id;
                    });

                    $qtyKirim = $matchedForecastDetail ? (float) $matchedForecastDetail->qty_forecast : 0;
    ...
    /**
     * Get pengiriman data based on filters (sama pola dengan buildIndexQuery status 'gagal' di PengirimanController)
     */
