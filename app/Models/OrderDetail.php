...
class OrderDetail extends Model
{
    ...
    {
        return $this->hasMany(OrderSupplier::class)->where(
            "is_available",
            true,
        );
    }
    ...
    {
        return $query->whereHas("orderSuppliers", function ($subQuery) use (
            $supplierId,
        ) {
    ...
    {
        $margin =
            $this->recommended_margin_percentage ??
            ($this->best_margin_percentage ?? 0);
     ...
     * Static Methods
     */
    public static function createFromPenawaran(
        Order $order,
        $penawaranDetail,
    ): self {
        $quantity = $penawaranDetail->quantity ?? ($penawaranDetail->qty ?? 0);
        $sellingPrice =
            $penawaranDetail->harga_klien ??
            ($penawaranDetail->harga_jual ?? 0);
        ...
        $detail = static::create([
            ...
            "catatan" =>
                $penawaranDetail->catatan ?? ($penawaranDetail->notes ?? null),
        ]);
        ...
        if ($penawaranDetail->bahan_baku_supplier_id ?? null) {
            ...
            $selectedSupplier = $detail
                ...
                ->where(
                    ...
                    $penawaranDetail->bahan_baku_supplier_id,
                )
                ->first();
        ...
        $uniqueSuppliers = $bahanBakuSuppliers
            ...
            ->map(function ($group) use ($materialName, $firstWord, $klienId) {
                ...
                $exactMatch = $group->first(function ($item) use (
                    ...
                    return strcasecmp($item->nama, $materialName) === 0;
                });
                if ($exactMatch) {
                    return $exactMatch;
                }

                // 2. Contains full material name (e.g., "Gaplek Chip" in "Gaplek Chip Premium")
                $fullNameMatch = $group->first(function ($item) use (
                    $materialName,
                ) {
                    return stripos($item->nama, $materialName) !== false;
                });
                if ($fullNameMatch) {
    ...
    public function updateSupplierSummary($suppliers = null): void
    {
        ...
        // Margin analysis
        $margins = $supplierCollection
            ->whereNotNull("calculated_margin")
            ->pluck("calculated_margin");
        ...
        if ($recommended) {
            ...
            $this->recommended_margin_percentage =
                $recommended->calculated_margin;
        }

        $this->available_suppliers_count = $supplierCollection
    ...
    public function updateFulfillmentTracking(): void
    {
        ...
        $this->remaining_quantity = max(
            0,
            $this->qty - $this->total_shipped_quantity,
        );
        $this->suppliers_used_count = $suppliers
            ->where("has_been_used", true)
            ->count();
