...
class EnhancedOrderSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        if (!$marketingUser) {
            $this->command->error('❌ Missing marketing user! Please run UserSeeder first.');
            return;
        }
        ...
        return [
            ...
            'details' => [
                [
                    ...
                    'satuan' => 'kg',
                    'harga_jual' => 12000,
                    'spesifikasi_khusus' => 'Pengiriman drum 200L, viskositas stabil',
                    'qty_shipped' => 2200,
    ...
    private function createOrderWithAutoSuppliers(array $orderData, $user): void
    {
        ...
        foreach ($orderData['details'] as $detailData) {
            $material = BahanBakuKlien::find($detailData['bahan_baku_klien_id']);

            if (!$material) {
                $this->command->warn("   ⚠️  Skipping detail - material ID {$detailData['bahan_baku_klien_id']} tidak ditemukan");
                continue;
            }

            $primarySupplier = BahanBakuSupplier::with('supplier')->find($detailData['primary_supplier_id']);

            if (!$primarySupplier) {
                $this->command->warn("   ⚠️  Skipping {$material->nama} - supplier ID {$detailData['primary_supplier_id']} tidak ditemukan");
                continue;
            }

            $qty = $detailData['qty'];
            $qtyShipped = $detailData['qty_shipped'] ?? 0;
            $remainingQty = $detailData['remaining_quantity'] ?? max(0, $qty - $qtyShipped);
            $detailStatus = $detailData['status'] ?? ($remainingQty <= 0 ? 'selesai' : ($qtyShipped > 0 ? 'sebagian_dikirim' : 'menunggu'));
            $totalShipped = $detailData['total_shipped_quantity'] ?? $qtyShipped;
            ...
            $this->command->line("   ✅ {$material->nama}: {$supplierCount} suppliers, best margin: {$bestMargin}%");
        }

        // Calculate order totals
        $order->calculateTotals();
        
        $totalSuppliers = $order->orderSuppliers()
    ...
    {
        return sprintf('PO-%s-%s-%s', strtoupper($prefix), Carbon::now()->format('Ymd'), strtoupper(Str::random(4)));
    }
    ...
    private function storeSamplePoDocument(string $poNumber): string
    {
        ...
        $disk->put($directory . '/' . $fileName, base64_decode(self::SAMPLE_PO_IMAGE_BASE64));
    ...
    {
        return 'Surat PO ' . strtoupper(str_replace('-', ' ', $poNumber)) . '.png';
    }
}
