@extends('layouts.app')
@section('title', 'Riwayat Harga Bahan Baku Klien - Kamil Maju Persada')
@section('content')


<x-welcome-banner title="Riwayat Harga Bahan Baku" :subtitle="$bahanBakuData->nama . ' untuk ' . $klienData->nama" icon="fas fa-chart-line" />
{{-- Breadcrumb --}}
<x-breadcrumb :items="[
    ['title' => 'Marketing', 'url' => '#'],
    ['title' => 'Klien', 'url' => route('klien.index')],
    ['title' => $klienData->nama, 'url' => route('klien.edit', $klienData->id)],
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        ...
        <div class="bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg p-3 sm:p-4 border border-purple-200">
            <div class="flex items-center mb-1 sm:mb-2">
                ...
                <span class="text-xs sm:text-sm font-semibold text-purple-700">Klien</span>
            </div>
            <p class="text-sm sm:text-lg font-bold text-gray-800">{{ $klienData->nama }} - {{ $klienData->cabang ?? '' }}</p>
        </div>
    </div>
</div>

{{-- Price Statistics Cards (reuse structure from supplier page) --}}
<div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 mb-6">
    ...
    <div class="bg-white rounded-lg shadow-sm p-3 sm:p-4 border-l-4 {{ $stats['trend_class'] ?? 'border-gray-500' }}">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-xs sm:text-sm font-medium text-gray-600">Trend ({{ $stats['days'] ?? 0 }} hari)</p>
                <p class="text-base sm:text-lg font-bold {{ $stats['trend_text_class'] ?? 'text-gray-600' }}">
                    {{ $stats['trend_prefix'] ?? '' }}{{ $stats['trend_percent'] ?? 0 }}%
                </p>
                <p class="text-xs text-gray-500 capitalize">{{ $stats['trend'] ?? 'stabil' }}</p>
            </div>
            <div class="p-2 sm:p-3 {{ $stats['trend_bg'] ?? 'bg-gray-100' }} rounded-full">
                <i class="fas fa-chart-line {{ $stats['trend_icon_class'] ?? 'text-gray-500' }} text-sm sm:text-base"></i>
            </div>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-lg sm:rounded-xl shadow-sm p-3 sm:p-6 border border-gray-200 mb-4 sm:mb-6">
    ...
    <div class="relative h-48 sm:h-64 lg:h-80">
        <canvas id="priceChartKlien" class="w-full h-full"></canvas>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!priceData || priceData.length === 0) {
        document.getElementById('priceChartKlien').style.display = 'none';
        const chartContainer = document.getElementById('priceChartKlien').parentElement;
    ...
    const ctx = document.getElementById('priceChartKlien').getContext('2d');
    ...
    new Chart(ctx, {
        ...
        data: {
            labels: labels,
            datasets: [{
                label: 'Harga per ' + (@json($bahanBakuData->satuan) || ''),
                data: prices,
        ...
        options: {
                ...
                tooltip: {
                    ...
                    callbacks: {
                        title: function(tooltipItems) {
                            ...
                            return fullDate.toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
                        },
                        label: function(context) { return 'Harga: Rp ' + context.parsed.y.toLocaleString('id-ID'); },
                        afterLabel: function(context) {
                            ...
                            if (dataIndex > 0) {
                                ...
                                if (change > 0) return `Naik: +Rp ${change.toLocaleString('id-ID')} (+${changePercent}%)`;
                                if (change < 0) return `Turun: Rp ${change.toLocaleString('id-ID')} (${changePercent}%)`;
            ...
            scales: {
                x: { grid: { color: 'rgba(156, 163, 175, 0.1)', drawBorder: false }, ticks: { color: 'rgb(107, 114, 128)', font: { size: 10 }, maxRotation: 45, minRotation: 45 } },
                y: { grid: { color: 'rgba(156, 163, 175, 0.1)', drawBorder: false }, ticks: { color: 'rgb(107, 114, 128)', font: { size: 11 }, callback: function(value){ return 'Rp ' + value.toLocaleString('id-ID'); } }, beginAtZero: false }
            },
