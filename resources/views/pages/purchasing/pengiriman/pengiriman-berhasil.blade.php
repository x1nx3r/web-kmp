    ...
    <div class="bg-white rounded-lg shadow-sm border">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex justify-between items-center">
                ...
                <a href="{{ route('pengiriman.export-berhasil', request()->all()) }}" 
        ...
        @if(isset($pengirimanBerhasil) && $pengirimanBerhasil->count() > 0)
                                    ...
                                    <div class="flex flex-col gap-1">
                                        ...
                                        @if(isset($pengiriman->partialInfo) && $pengiriman->partialInfo['isPartial'])
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-orange-100 text-orange-800 border border-orange-300 w-fit" 
                                                  title="Pengiriman sebagian: {{ $pengiriman->partialInfo['percentage'] }}% dari forecast ({{ number_format($pengiriman->partialInfo['totalQtyKirim'], 3, ',', '.') }} kg dari {{ number_format($pengiriman->partialInfo['totalQtyForecast'], 3, ',', '.') }} kg)">
            ...
            @if($pengirimanBerhasil->hasPages())
                @php
                    // Controller memakai pageName: berhasil_page
                    $pengirimanBerhasil->setPageName('berhasil_page');
                @endphp
                <div class="px-6 py-4 border-t border-gray-200">
                    <div class="flex items-center justify-between">
                        <div class="flex-1 flex justify-between sm:hidden">
                            ...
                            @else
                                <a href="{{ $pengirimanBerhasil->appends(array_merge(request()->except('berhasil_page'), ['tab' => 'pengiriman-berhasil']))->previousPageUrl() }}" class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 rounded-md hover:text-gray-500 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150">
                                    Sebelumnya
                                </a>
                            @endif

                            @if($pengirimanBerhasil->hasMorePages())
                                <a href="{{ $pengirimanBerhasil->appends(array_merge(request()->except('berhasil_page'), ['tab' => 'pengiriman-berhasil']))->nextPageUrl() }}" class="relative inline-flex items-center px-4 py-2 ml-3 text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 rounded-md hover:text-gray-500 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150">
                                    Selanjutnya
                                </a>
                            @else
                        ...
                        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            ...
                            <div>
                                {{ $pengirimanBerhasil->appends(array_merge(request()->except('berhasil_page'), ['tab' => 'pengiriman-berhasil']))->links() }}
                            </div>
                        </div>
                    </div>
                </div>
