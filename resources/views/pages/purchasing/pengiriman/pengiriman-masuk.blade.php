                    ...
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <div class="flex items-center space-x-3">
                            <div class="text-right">
                                ...
                                @if($totalHarga > 0)
                                    <p class="text-sm font-semibold text-green-600">Rp {{ number_format($totalHarga, 3, ',', '.') }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
                                            ...
                                            <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-600">
                                                ...
                                                @if($pengiriman->total_qty_kirim && $pengiriman->total_qty_kirim > 0)
                                                    <span class="flex items-center gap-1">
                                                        ...
                                                        {{ number_format($pengiriman->total_qty_kirim, 3, ',', '.') }} kg
                                                    </span>
                                                @endif
                                                @if($pengiriman->total_harga_kirim && $pengiriman->total_harga_kirim > 0)
                                                    <span class="flex items-center gap-1 font-medium text-green-700">
                                                        ...
                                                        Rp {{ number_format($pengiriman->total_harga_kirim, 3, ',', '.') }}
                                                    </span>
                                                @endif
                                            </div>
...
function calculateSubtotal(index) {
    ...
    if (qtyInput && hargaInput && totalInput && rowElement) {
        ...
        // Update total input
        totalInput.value = totalHargaBeli.toFixed(3);
...
function populateModalSummary(formData, modalElement) {
    ...
    if (detailContainer) {
            ...
            if (qty > 0) {
                ...
                const rowHtml = `
                    <tr class="border-b">
                        ...
                        <td class="px-3 py-2 text-sm">${qty.toLocaleString('id-ID', {minimumFractionDigits:3, maximumFractionDigits: 3})} kg</td>
                        <td class="px-3 py-2 text-sm">Rp ${harga.toLocaleString('id-ID', {minimumFractionDigits: 3, maximumFractionDigits: 3})}</td>
                        <td class="px-3 py-2 text-sm font-semibold">Rp ${total.toLocaleString('id-ID', {minimumFractionDigits: 3, maximumFractionDigits: 3})}</td>
                    </tr>
                `;
        ...
        if (setTotalQty) setTotalQty.textContent = totalQty.toLocaleString('id-ID', {minimumFractionDigits: 3, maximumFractionDigits: 3}) + ' kg';
        
        const setTotalHarga = modalElement.querySelector('#summary-total-harga');
        if (setTotalHarga) setTotalHarga.textContent = 'Rp ' + totalHarga.toLocaleString('id-ID', {minimumFractionDigits: 3, maximumFractionDigits: 3});
    }
...
function submitPengiriman() {
    ...
    detailItems.forEach((item, index) => {
        ...
        if (hargaBeliInput && totalHargaInput) {
            ...
            formData.set(`details[${index}][harga_satuan]`, hargaBeli.toFixed(3));
            formData.set(`details[${index}][total_harga]`, totalHarga.toFixed(3));
